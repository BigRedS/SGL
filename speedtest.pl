...
$filesize_down = ($filesize/1024)/1024; # convert to mb
$begin=[ Time::HiRes::gettimeofday() ];
...
open (LOGFILE, ">> $log");
	print "size up".$filesize;
	print "\nsize down".$filesize_down;
	print "\n";
#	print $time_since_epoch_m,"\t".$transfer_up."\t".$transfer_down."\t".$filesize."\t".$time_up."\t".$time_down."\t".$timestamp."\t".$time_now."\t".$date."\n";
close LOGFILE;
sleep 1;
